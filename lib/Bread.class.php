...
class Bread
{
    ...
    */
    public static function BreadCrumbs($url)
    {
       ...
       */
     // $bread_crumbs['0'] = $url[0];
        $bread_crumbs['1'] = $url[1];
        if(!empty($url[2])){
            if(is_numeric($url[2])) {
                $id = $url[2];
                $sql = "select name from goods where id_good = '$id'";
                $arr = db::getInstance()->Select($sql);
                $bread_crumbs['2'] = $arr[0]['name'];
            } else {
                $bread_crumbs['2'] = $url[2];
            }
        }
